...
RSpec.describe Talks::Operations::Approve do
  ...
  describe 'with mocks' do
    ...
    let(:talk_repo) { instance_double('TalkRepository', transaction: nil, find_unapproved: talk, update: true) }
    ...
    context 'when talk is not found' do
      before do
        allow(talk_repo).to receive(:find_unapproved).and_raise(ROM::TupleCountMismatchError)
      end
  ...
  describe 'with real data' do
    ...
    it 'creates all the entities' do
      subject
      expect(TalkRepository.new.find(id).state).to eq('approved')
      expect(TalkRepository.new.root.combine(:speakers).by_pk(id).speakers.map { |s| s[:state] }).to eq(['approved'])
      expect(TalkRepository.new.root.combine(:event).by_pk(id).one.event.state).to eq('approved')
    end
  end
end
